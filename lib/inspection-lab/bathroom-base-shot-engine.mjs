...
const BASELINE_DETECTION_MODEL =
  process.env.OPENAI_INSPECTION_BASELINE_MODEL || "gpt-4.1";
...
async function detectSingleTrackedObjectInPhotoWithAi(roomType, objectName, imageUrl, apiKey) {
  ...
  const response = await fetch(OPENAI_RESPONSES_URL, {
    ...
    body: JSON.stringify({
      model: BASELINE_DETECTION_MODEL,
      input: [
        {
          role: "user",
          content: [
...
export async function detectRoomObjectsInPhotoWithAi(roomType, imageInput) {
  ...
  const parsed = await parseResponsesOutput(response);
  const normalizedObjectChecks = Array.isArray(parsed?.objectChecks)
    ? parsed.objectChecks
        .map((item) => ({
          objectName: normalizeTrackedObjectName(item?.objectName, roomType),
  ...
  return {
    ...
    model: BASELINE_DETECTION_MODEL,
    roomType,
    objectChecks: normalizedObjectChecks,
