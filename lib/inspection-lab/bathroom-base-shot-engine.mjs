...
export async function detectRoomObjectsInPhotoWithAi(roomType, imageInput) {
  ...
  const prompt = [
    ...
    "Allowed object names are exactly:",
    trackedObjects.join(", "),
    "Return only objects that are likely visible in the image.",
    "For each visible object, include an approximate normalized center point in the image where x and y are each between 0 and 1.",
    "The coordinates should point near the visual center of that object in the image.",
    "If you are not reasonably confident about a location, use null for centerX and centerY.",
    "Return strict JSON only.",
    "JSON shape:",
    JSON.stringify(
      {
  ...
  return {
    ...
    model: DEFAULT_INSPECTION_MODEL,
    roomType,
    trackedObjects: Array.isArray(parsed?.trackedObjects)
      ? parsed.trackedObjects
          .map((item) => ({
            objectName: normalizeTrackedObjectName(item?.objectName, roomType),
            visibility:
              item?.visibility === "visible" || item?.visibility === "uncertain"
                ? item.visibility
                : "uncertain",
          ...
          .filter((item) => item.objectName !== "unknown")
      : [],
    summary: typeof parsed?.summary === "string" ? parsed.summary.trim() : "",
  };
}
