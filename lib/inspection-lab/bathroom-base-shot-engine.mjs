...
const TRACKED_BATHROOM_OBJECTS = ["sink", "mirror", "toilet", "bathtub_or_shower"];
...
export async function analyzeBathroomObjectsWithAi(
  ...
  const prompt = [
    ...
    "Focus only on these major bathroom objects: sink, mirror, toilet, bathtub_or_shower.",
    ...
    "Ignore shelf clutter and small toiletries unless they block a major object.",
  ...
  const trackedObjects = Array.isArray(parsed.trackedObjects)
    ? parsed.trackedObjects.map((item) => ({
        objectName:
          typeof item?.objectName === "string" && TRACKED_BATHROOM_OBJECTS.includes(item.objectName)
            ? item.objectName
            : "unknown",
