...
export async function sendMessengerMessage(
  ...
  const token = process.env.META_MESSENGER_ACCESS_TOKEN?.trim();

  if (!version || !input.channelAccountId.trim()) {
    return missingConfiguration("Messenger sending is not configured.");
  }
  if (!token) {
    return { ok: false, errorClass: "auth", message: "Messenger authentication is not configured." };
  }
